...
void printRules(Rules rules) {
    for (int i = 0; i < rules.size(); i++) {
        printf("RULE %d: /%d --> ", rules[i]->getIndex(), rules[i]->getFrom());
        int to;
...
bool isEndingInclusive(Closure *c1, Closure *c2) {
    ...
    while (it1 != items1.end()) {
        ...
        while(end2 != endings2.end()) {
            if (!endings1.count(*end2)) return false;
            end2++;
        }
        it1++;
        it2++;
    ...
    while(!next.empty()) {
        ...
        for (auto it = ids.begin(); it != ids.end(); it++) {
            ...
            if (visited.count(newClosure)) {
                if (!isEndingInclusive((*visited.find(newClosure)), newClosure)) {
